...
BEGIN
  ...
  INSERT INTO attendance_logs (
    student_lrn, check_in_time, check_out_time, date, is_present,
    attendance_status, cancellation_status, is_late, is_invalid_timeout,
    activity_name, is_auto_generated
  )
  SELECT
    s.lrn,
    ...
    p_activity_date,
    CASE WHEN existing.check_in_time IS NOT NULL
          AND existing.is_auto_generated IS DISTINCT FROM TRUE THEN TRUE ELSE FALSE END,
    CASE WHEN existing.check_in_time IS NOT NULL
